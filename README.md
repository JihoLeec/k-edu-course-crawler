...
여친이 강의 정보를 엑셀에 열심히 복붙하고 있길래, 소프트웨어 엔지니어로서 참을 수 없어서 만들었습니다.  
...
가능합니다. 엑셀 `K-MOOC` 시트는 KOCW와 **같은 컬럼 구조**이고,  
소스는 [K-MOOC](https://www.kmooc.kr/) / 사이트만 다릅니다.

구조상으로는 `crawl_kmooc.py`를 두고 시트만 `K-MOOC`에 쓰면 두 크롤러가 공존합니다.  
(이 저장소에는 아직 K-MOOC용 스크립트가 없고, 시트 양식만 남아 있습니다.)
